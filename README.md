...
**矛盾語の扱いについて:** 名詞編と用言編の統合時に評価が2つに割れた3語（賛成・規律・買い得です）は、どちらか一方に決めつけず**両方の値を提示**します。これらの語を含む分析結果は「下限〜上限」の範囲で表示され、辞書一覧・検出語リストでも両スコアを併記します。詳細は [dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md) を参照してください。
...
- 同梱辞書の重複13行を整理（18,541語 → 18,528語）。評価が2つに割れる3語（賛成・規律・買い得です）は片方に決めず両方の値を提示し、分析結果は範囲表示にした（[dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md)）
