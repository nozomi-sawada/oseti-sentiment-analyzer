...
- **`dictionaries/japanese_sentiment_dictionary.txt`** - 完全版辞書（名詞編+用言編）
  - 18,541行 / ユニーク18,528語（元辞書の値をそのまま保持）
  - そのまま使用可能
...
**矛盾語の扱いについて:** 同梱辞書は**元の辞書の値をそのまま保持**しています（値の変更・削除はありません）。名詞編と用言編で評価が2つに割れた3語（賛成・規律・買い得です）は元辞書のとおり2行で収録され、ツールがそれを検出して**両方の値を提示**します。これらの語を含む分析結果は「下限〜上限」の範囲で表示されます。詳細は [dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md) を参照してください。
...
<sub>※ 評価が割れる3語（賛成・規律・買い得です）は代表値（元辞書での最初の出現）で集計。分析では両値を範囲表示します。</sub>
...
- 同梱辞書は元辞書の値をそのまま保持（変更・削除なし）。評価が2つに割れる3語（賛成・規律・買い得です）を辞書データから自動検出し、片方に決めず両方の値を提示、分析結果は範囲表示にした（[dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md)）
