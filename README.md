...
**矛盾語の扱いについて:** 名詞編と用言編の統合時に評価が矛盾していた3語のうち、明らかな誤り1語（買い得です）のみ訂正し、極性を判断できない2語（賛成・規律）は中立（0.0）として扱っています。いずれも元辞書の両スコアを併記しています。詳細は [dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md) を参照してください。ツールの「辞書一覧」タブでも、これらの語に元辞書の記載が併記されます。
...
- 同梱辞書の重複13行を整理（18,541語 → 18,528語）。評価が矛盾していた3語は、明らかな誤り1語（買い得です）のみ訂正し、判断できない2語（賛成・規律）は中立とした上で元辞書の両スコアを併記（[dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md)）
