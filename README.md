...
**補正について:** 名詞編と用言編の統合時に評価が矛盾していた3語（賛成・規律・買い得です）は、元辞書の両スコアを併記した上で妥当な値に補正しています。詳細は [dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md) を参照してください。ツールの「辞書一覧」タブでも、これらの語に元辞書の記載が併記されます。
...
- 同梱辞書の重複13行を整理（18,541語 → 18,528語）。評価が矛盾していた3語（賛成・規律・買い得です）は元辞書の両スコアを併記した上で妥当な値に補正（[dictionaries/CORRECTIONS.md](dictionaries/CORRECTIONS.md)）
